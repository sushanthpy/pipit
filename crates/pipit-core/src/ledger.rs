...
/// A single todo item — the model replaces the entire list each call.
///
/// Shape follows abc-src's TodoWrite: `content` is the task description,
/// `status` is one of three states, `active_form` is the TUI display text
/// (e.g. "Running tests…") generated by the model.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct TodoItem {
    pub content: String,
    /// Human-readable status string for TUI display (e.g. "Running tests…").
    #[serde(default)]
...
pub enum SessionEvent {
    ...
    /// Task 9: Enriched SubagentCompleted with full transcript data.
    SubagentCompleted {
        child_id: String,
        success: bool,
        /// Full output text from the child.
        #[serde(default)]
    ...
    // ── Todo (Task 5: replace-all semantics) ──
    /// Replace the entire todo list. Last-write-wins projection.
    /// Auto-clears when all items are completed (abc's trick).
    TodoWrite {
        todos: Vec<TodoItem>,
    },

    // ── Verification nudge (Task 6) ──
    VerificationNudgeFired {
        todo_count: usize,
        is_coordinator: bool,
    },
...
pub struct SessionState {
    ...
    /// Task 5: Current todo list — projection of TodoWrite events.
    /// `current_todos = fold(events.filter(TodoWrite), |_, e| e.todos)` (last-write-wins).
    #[serde(default)]
    pub todos: Vec<TodoItem>,

    /// Task 6: Count of verification nudges fired this session.
    #[serde(default)]
    pub verification_nudges_fired: u32,

    /// Task 9: Completed subagent transcripts (for evidence chain).
    #[serde(default)]
    pub subagent_transcripts: Vec<SubagentTranscriptSummary>,
}

/// Compact summary of a subagent execution for SessionState projection.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
impl SessionState {
    ...
    pub fn reduce(&mut self, event: &LedgerEvent) {
        ...
        match &event.payload {
            ...
            // Task 9: Enriched SubagentCompleted — populate transcript summary
            SessionEvent::SubagentCompleted {
                ...
                self.subagent_transcripts.push(SubagentTranscriptSummary {
            ...
            // ── Todo (Task 5): last-write-wins projection ──
            SessionEvent::TodoWrite { todos } => {
                // Auto-clear on all-done (abc's trick): if every item is completed,
                // store [] instead to prevent the model from shipping stale lists.
                if !todos.is_empty()
                    && todos
                        .iter()
            ...
            // ── Verification nudge (Task 6) ──
            SessionEvent::VerificationNudgeFired { .. } => {
                self.verification_nudges_fired += 1;
            }
