...
use crate::spec_lang::{CompareOp, ConstraintValue, Spec, SpecConstraint};
    ...
    for rule in &spec.rules {
        ...
        if !rule_conflicts.is_empty() {
            conflicts.push(format!(
                "Rule '{}' has contradictory precondition: {}",
                rule.name,
                rule_conflicts.join(", ")
            ));
        }
...
mod tests {
    ...
    fn test_inconsistent_spec() {
        ...
        assert!(
            !result.is_consistent,
            "x ∈ [0,5] ∧ x≥10 should be inconsistent"
        );
    }
}
